import type { GpuInfo, Tier, Vendor } from "./types";
...
function detectVendor(name: string): Vendor {
  ...
  if (n.includes("nvidia") || /\b(rtx|gtx)\b/.test(n)) return "nvidia";
  if (n.includes("amd") || n.includes("radeon") || /\brx\s?\d/.test(n)) return "amd";
 ...
 */
export function tierForModel(rawName: string): Tier {
  ...
  if (vendor === "nvidia") {
    ...
    if (/gtx/.test(n)) return 1;
    // Datacenter / workstation cards newer than Ampere -> Tier 2.
    if (/\b(a100|h100|h200|l40|l4|a40|a10|rtx a\d{4})\b/.test(n)) return 2;
    return 1;
  }
...
export function classifyGpu(rawName: string, memoryMb?: number): GpuInfo {
  const vendor = detectVendor(rawName);
